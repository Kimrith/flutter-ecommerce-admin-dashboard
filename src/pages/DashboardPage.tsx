...
export const DashboardPage: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="glass-card p-5 sm:p-6 rounded-2xl sm:rounded-3xl border border-rose-500/20 bg-gradient-to-r from-slate-900 via-slate-900 to-rose-950/40 flex flex-col md:flex-row items-start md:items-center justify-between gap-5 sm:gap-6">
        <div className="flex items-start sm:items-center gap-4 sm:gap-5">
          <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-gradient-to-tr from-rose-600 to-red-500 flex items-center justify-center text-white shadow-xl shadow-rose-600/30 shrink-0 font-black text-xs sm:text-sm">
            KHQR
          </div>
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="text-base sm:text-lg font-bold text-white">Bakong KHQR Payment Gateway</h3>
        ...
        <Link
          to="/admin/payments"
          className="inline-flex items-center justify-center gap-2 w-full sm:w-auto px-5 py-3 bg-rose-600 hover:bg-rose-500 text-white font-bold rounded-xl text-sm shadow-lg shadow-rose-600/25 transition-all shrink-0"
        >
          ...
          <table className="w-full text-left border-collapse min-w-[620px]">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-900/50 text-[11px] font-bold uppercase tracking-wider text-slate-400">
                <th className="px-4 sm:px-6 py-3.5">Order Number</th>
                <th className="px-4 sm:px-6 py-3.5">Total Amount</th>
                <th className="px-4 sm:px-6 py-3.5">Status</th>
                <th className="px-4 sm:px-6 py-3.5">Payment Method</th>
              ...
              {ordersList.length === 0 ? (
                ...
                ordersList.slice(0, 5).map((order) => (
                  <tr key={order.id} className="hover:bg-slate-800/40 transition-colors">
                    <td className="px-4 sm:px-6 py-3.5 sm:py-4 font-semibold text-white">
                      #{order.orderNumber || order.id.slice(0, 8)}
                    </td>
                    <td className="px-4 sm:px-6 py-3.5 sm:py-4 font-bold text-emerald-400">
                      ${order.totalAmount?.toFixed(2)}
                    </td>
                    <td className="px-4 sm:px-6 py-3.5 sm:py-4">
                      <StatusBadge type="order" status={order.status} />
                    </td>
                    <td className="px-4 sm:px-6 py-3.5 sm:py-4 text-slate-300 font-medium text-xs">
                      {order.payment?.paymentMethod || 'BAKONG_KHQR'}
                    </td>
                    <td className="px-4 sm:px-6 py-3.5 sm:py-4 text-slate-400 text-xs flex items-center gap-1.5">
